...
describe("FP - array functions", function() {
  ...
  function fixProjects(student) {
    let {projects = ""} = student; //normalizacia
    return {
      ...student,
  ...
  it("04-find students with less then 3 projects", () => {
    ...
    //filter, odfiltrujeme < 3 projekty
  ...
  function fixPoints(student) {
    return {
      ...
      //map, prerobit strukturu bodov
      points: Object.entries(student).filter(([k,v]) => k.includes("point"))
      .map(([k,v]) => v)
    }
  }
  ...
  it("05-restructire points", () => {
    ...
    //reduce, poitns su array
    //Object keys, filternem ho cez tie co zacinaju na points - nazvy properties - transformujem array na array pomocou map, object si posleme ako this
  ...
  function totalPoints(student) {
    return student.points.reduce((x,y) => x + y, 0)
  }
  ...
  it("10-projects with more students", () => {
    ...
    //dopisat filter k tomu predchadzajucemu, filter podla poctu studentov
